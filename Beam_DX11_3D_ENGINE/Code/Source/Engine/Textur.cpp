#include "..\..\Include\Engine\Textur.h"

#define STB_IMAGE_IMPLEMENTATION
#include <stb_image.h>

bool CTextur::LoadTextureArrayFromFiles(const std::vector<std::string>& filenames, Texture2DDescriptor & outImage)
{
	try
	{
		...
		for (unsigned int k = 0; k < outImage.depth; ++k)
		{
			...
			outImage.inByteSize = w * h * 4 * sizeof(uint8_t);
...
bool CTextur::LoadTextureFromFile(const char * filename, Texture2DDescriptor & outImage)
{
	try
	{
		int w = 0, h = 0, c = 0;
